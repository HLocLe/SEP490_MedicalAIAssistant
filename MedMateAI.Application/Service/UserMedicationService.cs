...
public sealed class UserMedicationService : IUserMedicationService
{
    ...
    public async Task<UserMedicationOperationResult<PagedResponse<UserMedicationResponse>>>
        GetMinePagedAsync(
            ...
            CancellationToken cancellationToken = default)
    {
        var medications = await _unitOfWork.UserMedications.GetPagedByUserIdAsync(
            userId,
            query.PageNumber,
            query.PageSize,
            cancellationToken);

        return UserMedicationOperationResult<PagedResponse<UserMedicationResponse>>.Ok(
            PagedResponse<UserMedicationResponse>.From(medications, Map));
    }
